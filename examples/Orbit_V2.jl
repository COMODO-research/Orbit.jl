...
const MM_TO_CM = 0.1
const OVERHANG_THRESHOLD_DEGREES = 45.0
...
"""
    load_mesh_from_file(test_case::Int)
...
function load_mesh_from_file(test_case::Int)
    ...
    elseif test_case == 2
        "C:\\Users\\24242721\\Downloads\\Cervical Cage v1.0 v2.stl"
    elseif test_case == 3
...
"""
    create_test_orientations(subdivisions::Int, radius::Float64=1.0)
...
"""
    create_pattern_sequence(vertices, pattern_type::String)

Create different traversal patterns for geosphere orientations.

# Patterns
- "Z-Sort (Top to Bottom)": Descending by Z-coordinate
- "Longitude Sweep": Angular sweep around vertical axis
- "Latitude Spiral": Alternating horizontal bands

Returns a vector of indices defining traversal order
"""
function create_pattern_sequence(vertices, pattern_type)
    n = length(vertices)
    z_coords = [v[3] for v in vertices]
    angles = [atan(v[2], v[1]) for v in vertices]

    if pattern_type == "Z-Sort (Top to Bottom)"
        return sortperm(z_coords, rev=true)
        
    elseif pattern_type == "Longitude Sweep"
        # Sort by angle, then by descending height
        combined = [(angles[i], -z_coords[i]) for i in 1:n]
        return sortperm(combined)
        
    elseif pattern_type == "Latitude Spiral"
        band_width = 0.4 
        bands = round.(Int, z_coords ./ band_width)
        spiral_order = Int[]
        unique_bands = sort(unique(bands), rev=true)  # Top to bottom
        
        # Alternate sweep direction per band to create spiral
        for (band_idx, band) in enumerate(unique_bands)
            band_indices = findall(==(band), bands)
            band_angles = angles[band_indices]
            
            # Alternate sorting direction
            sorted_indices = sortperm(band_angles, rev=iseven(band_idx))
            append!(spiral_order, band_indices[sorted_indices])
        end
        return spiral_order
    else
        return collect(1:n)  # Default: natural order
    end
end

"""
    orient_part_for_printing(vertices, build_direction)
...
"""
    calculate_bounding_box_metrics(vertices)

Calculate axis-aligned bounding box volume and edges for visualisation.
Returns volume in mm^3 and 12 edge lines for wireframe display.
"""
...
"""
    calculate_shadow_projection_area(faces, vertices, grid_resolution=0.5)
...
"""
    calculate_up_facing_area(faces, vertices, angle_threshold=45.0)
...
"""
    calculate_down_facing_area(faces, vertices, angle_threshold=45.0)
...
"""
    calculate_gravity_center_height(vertices)
...
"""
    identify_support_requirements(faces, vertices, overhang_threshold=OVERHANG_THRESHOLD_DEGREES, support_diameter=SUPPORT_DIAMETER_MM, 
                                 density_reduction=SUPPORT_DENSITY_REDUCTION, support_type="Lattice")
...
"""
function identify_support_requirements(faces, vertices, overhang_threshold=OVERHANG_THRESHOLD_DEGREES, support_diameter=SUPPORT_DIAMETER_MM, 
                                      density_reduction=SUPPORT_DENSITY_REDUCTION, support_type="Lattice")
...
"""
    generate_lattice_supports!(support_lines, support_columns, column_heights, x_points, y_points, grid_spacing)
...
"""
    find_internal_supports!(support_lines, face_indices, faces, vertices, normals, x_points, y_points, z_min, threshold_rad)
...
"""
    calculate_support_material_volumes(external_supports, internal_supports; support_diameter=SUPPORT_DIAMETER_MM)
...
"""
    slice_mesh_at_z_height(faces, vertices, z_height)
...
"""
    find_triangle_plane_intersections(faces, vertices, plane_point, plane_normal)
...
function find_triangle_plane_intersections(faces, vertices, plane_point, plane_normal)
    ...
    for face in faces
        ...
        # Add segment if there is exactly 2 points
...
"""
    build_contours_from_segments(segments)
...
"""
    calculate_contour_area(contours)
...
"""
    calculate_contour_perimeter(contours)
...
"""
    find_max_xy_cross_section(faces, vertices, num_slices=50)
...
"""
    create_cutting_plane_mesh(vertices, z_height, margin_factor=0.2)
...
"""
    apply_z_offset(vertices, z_offset)
...
"""
    count_supports_cut(support_lines, plane_point, plane_normal)
...
function calculate_optimal_bandsaw_plane(vertices, support_lines, z_offset=0.0)
    ...
    clearance = 1.0  # Minimum distance from part
...
"""
    analyse_support_distribution(support_lines)
...
"""
    optimise_angled_cuts(support_lines, support_bounds, offset_vertices, clearance, current_best_plane, current_best_count, current_best_angle)
...
"""
    calculate_angled_plane(direction, angle_rad, support_bounds, base_height)
...
"""
    minimum_part_distance(vertices, plane_point, plane_normal)
...
"""
    create_bandsaw_plane_mesh(plane_data, vertices, z_offset=0.0)
...
"""
    normalise_metric_values(raw_values)
...
"""
    calculate_weighted_scores(heights, shadows, volumes, max_crosssections, up_facing_areas, down_facing_areas, 
...
"""
    export_oriented_stl!(faces, verts, outpath)

Export the oriented mesh (current orientation only) as STL.
"""
function export_oriented_stl!(faces::Vector, verts::Vector{Point{3,Float64}}, outpath::AbstractString)
...
"""
    update_slice_visualisation(orientation_index, z_height, oriented_meshes, part_faces, slice_area_obs, slice_plane_vis, slice_contour_data_obs, show_slice_plane_obs, slice_axis)  

Update slice visualisation including contours and cutting plane.
"""
function update_slice_visualisation(orientation_index, z_height, oriented_meshes, part_faces, slice_area_obs, slice_plane_vis, slice_contour_data_obs, show_slice_plane_obs, slice_axis)
...
"""
    create_spider_chart_interactive(axis, metric_values_obs, metric_names, metric_colors)
...
"""
    analyse_all_orientations(test_case::Int, analysis_resolution=1.0, slice_resolution=50)
...
"""
    create_interactive_interface(part_faces, part_vertices, orientation_faces, orientation_vertices, 
                                ...
                                external_supports, internal_supports, scores, optimal_orientations, test_case)
...
function create_interactive_interface(part_faces, part_vertices, orientation_faces, orientation_vertices, 
    ...
    main_figure = Figure(size=(1920, 1080), backgroundcolor=:white)
    main_controls_layout = GridLayout(main_figure[1:4, 1], tellwidth=true, width=300)
    ...
    current_pattern = Observable("Z-Sort (Top to Bottom)")
    pattern_sequence_obs = Observable(create_pattern_sequence(orientation_vertices, "Z-Sort (Top to Bottom)"))
    is_auto_playing = Observable(false)
    
    # ========================================================================
    # LAYOUT STRUCTURE
    # ========================================================================
    part_view = AxisGeom(main_figure[1:3, 2:5], title="Live Orientation", aspect=:data)
    cross_section_view = AxisGeom(main_figure[1:3, 5:8], title="Cross-Section View", aspect=:data)
    metrics_layout = GridLayout(main_figure[1:3, 8], tellwidth=true, width=200)
    performance_sphere_view = AxisGeom(main_figure[4, 6], title="Performance Sphere", aspect=:data)
    opposite_hemisphere_view = AxisGeom(main_figure[4, 7], title="Opposite Hemisphere", aspect=:data)
    performance_radar_view = Axis(main_figure[4, 8], title="Performance Radar")
    metrics_history_layout = GridLayout(main_figure[4, 1:5])
    metrics_history_view = Axis(metrics_history_layout[4, 2:4], title="Orientation Scanner")
    metrics_toggles_layout = GridLayout(metrics_history_layout[4, 5], tellwidth=true, width=100)
    
    # Coordinate systems
    coord_system_view = AxisGeom(main_figure[1, 2], aspect=:data, width=150, height=150)
    hidedecorations!(coord_system_view)
    hidespines!(coord_system_view)
    
    sync_coord_view = AxisGeom(main_figure[1, 5], aspect=:data, width=150, height=150)
    hidedecorations!(sync_coord_view)
    hidespines!(sync_coord_view)
    
    # ========================================================================
    # COORDINATE SYSTEMS
    # ========================================================================
    origin = Point{3,Float64}(0.0, 0.0, 0.0)
    x_end = Point{3,Float64}(1.0, 0.0, 0.0)
    y_end = Point{3,Float64}(0.0, 1.0, 0.0)
    z_end = Point{3,Float64}(0.0, 0.0, 1.0)
    
    # Fixed coordinate system
    lines!(coord_system_view, [origin, x_end], color=:red, linewidth=4)
    lines!(coord_system_view, [origin, y_end], color=:green, linewidth=4)
    lines!(coord_system_view, [origin, z_end], color=:blue, linewidth=4)
    
    # Arrowheads
    x_arrow_tip1 = Point{3,Float64}(0.85, 0.08, 0.0)
    x_arrow_tip2 = Point{3,Float64}(0.85, -0.08, 0.0)
    lines!(coord_system_view, [x_end, x_arrow_tip1], color=:red, linewidth=3)
    lines!(coord_system_view, [x_end, x_arrow_tip2], color=:red, linewidth=3)
    
    y_arrow_tip1 = Point{3,Float64}(0.08, 0.85, 0.0)
    y_arrow_tip2 = Point{3,Float64}(-0.08, 0.85, 0.0)
    lines!(coord_system_view, [y_end, y_arrow_tip1], color=:green, linewidth=3)
    lines!(coord_system_view, [y_end, y_arrow_tip2], color=:green, linewidth=3)
    
    z_arrow_tip1 = Point{3,Float64}(0.08, 0.0, 0.85)
    z_arrow_tip2 = Point{3,Float64}(-0.08, 0.0, 0.85)
    lines!(coord_system_view, [z_end, z_arrow_tip1], color=:blue, linewidth=3)
    lines!(coord_system_view, [z_end, z_arrow_tip2], color=:blue, linewidth=3)
    
    text!(coord_system_view, Point{3,Float64}(1.15, 0.0, 0.0), text="X\nGas Flow", fontsize=6, color=:red, align=(:center, :center), font="Arial Bold")
    text!(coord_system_view, Point{3,Float64}(0.0, 1.15, 0.0), text="Y\nRecoater", fontsize=6, color=:green, align=(:center, :center), font="Arial Bold")
    text!(coord_system_view, Point{3,Float64}(0.0, 0.0, 1.25), text="Z\nBuild Direction", fontsize=6, color=:blue, align=(:center, :center), font="Arial Bold")
    
    xlims!(coord_system_view, -0.3, 1.5)
    ylims!(coord_system_view, -0.3, 1.5)
    coord_system_view.elevation = 0.3
    coord_system_view.azimuth = 0.7
    
    # Synchronised coordinate system
    sync_x_line = lines!(sync_coord_view, [origin, x_end], color=:red, linewidth=3)
    sync_y_line = lines!(sync_coord_view, [origin, y_end], color=:green, linewidth=3)
    sync_z_line = lines!(sync_coord_view, [origin, z_end], color=:blue, linewidth=3)
    
    sync_x_text = text!(sync_coord_view, Point{3,Float64}(1.1, 0.0, 0.0), text="X", fontsize=10, color=:red, align=(:center, :center), font="Arial Bold")
    sync_y_text = text!(sync_coord_view, Point{3,Float64}(0.0, 1.1, 0.0), text="Y", fontsize=10, color=:green, align=(:center, :center), font="Arial Bold")
    sync_z_text = text!(sync_coord_view, Point{3,Float64}(0.0, 0.0, 1.1), text="Z", fontsize=10, color=:blue, align=(:center, :center), font="Arial Bold")
    
    xlims!(sync_coord_view, -0.3, 1.3)
    ylims!(sync_coord_view, -0.3, 1.3)
    sync_coord_view.elevation = 0.3
    sync_coord_view.azimuth = 0.7
    
    # ========================================================================
    # METRICS DISPLAY
    # ========================================================================
    metrics_labels = []
    metrics_names = ["Height", "Shadow", "Volume", "Max XY", "Up-Face", "Down-Face", "Gravity", "Ext.Sup", "Int.Sup", "Combined", "Z Offset", "Supports"]
    
    for (i, label_name) in enumerate(metrics_names)
        Label(metrics_layout[i, 1], text=label_name, fontsize=9, halign=:right, width=60)
        metric_label = Label(metrics_layout[i, 2], text="", fontsize=8, color=:blue, halign=:left, width=180)
        push!(metrics_labels, metric_label)
    end
    
    # Logos
    Label(metrics_layout[length(metrics_names) + 1, 1:2], "", height=10)
    Label(metrics_layout[length(metrics_names) + 2, 1:2], "Powered by:", fontsize=8, halign=:center)
    logo_grid = GridLayout(metrics_layout[length(metrics_names) + 3, 1:2])
    
    logo_files = ["University_of_Galway_logo_2022.png", "i-form-logo-1.png", "ASMME_logo.png", "Comodo_logo.png"]
    logo_size = 120
    ...
    for (idx, logo_path) in enumerate(logo_files)
        ...
        col = mod(idx - 1, 2) + 1
        try
            logo_img = load(logo_path)
            ax_logo = Axis(logo_grid[row, col], width=logo_size, height=logo_size, aspect=DataAspect())
            image!(ax_logo, rotr90(logo_img))
            hidedecorations!(ax_logo)
            hidespines!(ax_logo)
        catch e
            Label(logo_grid[row, col], "Logo $idx", fontsize=8, width=logo_size, height=logo_size, halign=:center, valign=:center, color=:gray)
        end
    end
    ...
    bandsaw_plane_visualisation = meshplot!(part_view, initial_empty_mesh, color=(:orange, 0.4), strokewidth=3, strokecolor=(:red, 0.8))
    ...
    # History plots
    history_plots = []
    metric_visibility = [Observable(true) for _ in 1:9]
    
    orientation_indices_display = @lift begin
        seq = $pattern_sequence_obs
        1:length(seq)
    end
    
    for (i, (name, color)) in enumerate(zip(metric_names, metric_colors))
        metric_data_reordered = @lift begin
            seq = $pattern_sequence_obs
            [norm_metrics[i][seq[j]] for j in 1:length(seq)]
        end
        
        line_plot = lines!(metrics_history_view, orientation_indices_display, metric_data_reordered, 
    ...
    Label(main_controls_layout[row, 1], "Geosphere Pattern:", fontsize=10, halign=:left)
    row += 1
    pattern_dropdown = Menu(main_controls_layout[row, 1], 
        options=["Z-Sort (Top to Bottom)", "Longitude Sweep", "Latitude Spiral"], 
        default="Z-Sort (Top to Bottom)", width=240)
    row += 1
    
    Label(main_controls_layout[row, 1], "", fontsize=3)
    row += 1
    
    auto_play_button = Button(main_controls_layout[row, 1], label="Auto Play", width=240)
    row += 1
    
    save_orientation_button = Button(main_controls_layout[row, 1], label="Save Orientation Report", width=240)
    row += 1
    
    on(save_orientation_button.clicks) do _
        current_seq = pattern_sequence_obs[]
        current_pos = orientation_control.value[]
        actual_orientation_index = current_seq[current_pos]
        
        timestamp = replace(string(now()), ":" => "-", "." => "-")
        report_fig = Figure(size=(800, 1200), backgroundcolor=:white)
        
        Label(report_fig[1, 1:3], "3D Print Orientation Analysis Report", 
              fontsize=32, color=:darkblue, halign=:center, font=:bold)
        
        metadata_text = "Generated: $(now()) | Test Case: $test_case | Pattern: $(current_pattern[]) | Position: $current_pos"
        Label(report_fig[2, 1:3], metadata_text, fontsize=13, halign=:center)
        
        config_text = "Z-Offset: $(round(z_offset[], digits=1))mm | Support Type: $(support_type[]) | View: $(mode_dropdown.selection[])"
        Label(report_fig[3, 1:3], config_text, fontsize=13, halign=:center, color=:gray30)
        
        Box(report_fig[4, 1:3], color=:darkblue, height=4)
        Label(report_fig[5, 1:3], "", height=15)
        
        header_row = GridLayout(report_fig[6, 1:3])
        h1_grid = GridLayout(header_row[1, 1])
        Label(h1_grid[1, 1], "Optimal Orientations          ", fontsize=18, font=:bold, halign=:left, color=:darkblue)
        Label(header_row[1, 2], "Current Metrics", fontsize=18, font=:bold, halign=:center, color=:darkblue)
        h3_grid = GridLayout(header_row[1, 3])
        Label(h3_grid[1, 1], "          Optimisation Weights", fontsize=18, font=:bold, halign=:left, color=:darkblue)
        
        Box(header_row[2, 1], color=:darkblue, height=3)
        Box(header_row[2, 2], color=:darkblue, height=3)
        Box(header_row[2, 3], color=:darkblue, height=3)
        Label(report_fig[7, 1:3], "", height=10)
        
        col1 = GridLayout(report_fig[8, 1], tellwidth=true, width=250)
        col2 = GridLayout(report_fig[8, 2], tellwidth=true, width=250)
        col3 = GridLayout(report_fig[8, 3], tellwidth=true, width=250)
        
        function get_pattern_position(orientation_idx)
            findfirst(x -> x == orientation_idx, current_seq)
        end
        
        optimal_data = [
            ("Height", get_pattern_position(optimal_orientations.height), round(heights[optimal_orientations.height], digits=2), "mm"),
            ("Shadow", get_pattern_position(optimal_orientations.shadow), round(shadows[optimal_orientations.shadow] * MM_TO_CM^2, digits=2), "cm²"),
            ("Volume", get_pattern_position(optimal_orientations.volume), round(volumes[optimal_orientations.volume] * MM_TO_CM^3, digits=2), "cm³"),
            ("MaxXY", get_pattern_position(optimal_orientations.max_crosssection), round(max_crosssections[optimal_orientations.max_crosssection] * MM_TO_CM^2, digits=2), "cm²"),
            ("Up-Face", get_pattern_position(optimal_orientations.up_facing_area), round(up_facing_areas[optimal_orientations.up_facing_area] * MM_TO_CM^2, digits=2), "cm²"),
            ("Down", get_pattern_position(optimal_orientations.down_facing_area), round(down_facing_areas[optimal_orientations.down_facing_area] * MM_TO_CM^2, digits=2), "cm²"),
            ("Gravity", get_pattern_position(optimal_orientations.gravity_center), round(gravity_centres[optimal_orientations.gravity_center], digits=2), "mm"),
            ("ExtSup", get_pattern_position(optimal_orientations.external_support), round(external_volumes[optimal_orientations.external_support] * MM_TO_CM^3, digits=2), "cm³"),
            ("IntSup", get_pattern_position(optimal_orientations.internal_support), round(internal_volumes[optimal_orientations.internal_support] * MM_TO_CM^3, digits=2), "cm³"),
            ("Score", get_pattern_position(best_combined_index[]), round(current_scores[][best_combined_index[]], digits=2), "")
        ]
        
        row_idx = 1
        for (label, pos, value, unit) in optimal_data
            row_grid = GridLayout(col1[row_idx, 1])
            Label(row_grid[1, 1], "$(rpad(label, 9)): ", fontsize=14, halign=:left, width=85, font=:bold)
            Label(row_grid[1, 2], "#$pos ", fontsize=14, halign=:right, color=:blue, width=40, font=:bold)
            Label(row_grid[1, 3], "($value$unit)", fontsize=14, halign=:left, color=:gray30)
            row_idx += 1
            Label(col1[row_idx, 1], "", height=8)
            row_idx += 1
        end
        
        Label(col2[1, 1], "Pattern Position: $current_pos", fontsize=15, halign=:center, font=:bold, color=:red)
        Label(col2[2, 1], "", height=8)
        
        current_metrics = [
            ("Height", round(heights[actual_orientation_index], digits=2), "mm"),
            ("Shadow", round(shadows[actual_orientation_index] * MM_TO_CM^2, digits=2), "cm²"),
            ("Volume", round(volumes[actual_orientation_index] * MM_TO_CM^3, digits=2), "cm³"),
            ("MaxXY", round(max_crosssections[actual_orientation_index] * MM_TO_CM^2, digits=2), "cm²"),
            ("Up-Face", round(up_facing_areas[actual_orientation_index] * MM_TO_CM^2, digits=2), "cm²"),
            ("Down", round(down_facing_areas[actual_orientation_index] * MM_TO_CM^2, digits=2), "cm²"),
            ("Gravity", round(gravity_centres[actual_orientation_index], digits=2), "mm"),
            ("ExtSup", round(external_volumes[actual_orientation_index] * MM_TO_CM^3, digits=2), "cm³"),
            ("IntSup", round(internal_volumes[actual_orientation_index] * MM_TO_CM^3, digits=2), "cm³"),
            ("Score", round(current_scores[][actual_orientation_index], digits=2), "")
        ]
        
        row_idx = 3
        for (label, value, unit) in current_metrics
            row_grid = GridLayout(col2[row_idx, 1])
            Label(row_grid[1, 1], "$(rpad(label, 9)): ", fontsize=14, halign=:left, width=85, font=:bold)
            Label(row_grid[1, 2], "$value$unit", fontsize=14, halign=:left, color=:gray30)
            row_idx += 1
            Label(col2[row_idx, 1], "", height=8)
            row_idx += 1
        end
        
        Label(col2[row_idx, 1], "", height=10)
        row_idx += 1
        Label(col2[row_idx, 1], "Support Structures", fontsize=14, font=:bold, halign=:center, color=:darkblue)
        row_idx += 1
        Label(col2[row_idx, 1], "External: $(length(external_supports[actual_orientation_index])) | Internal: $(length(internal_supports[actual_orientation_index]))", 
              fontsize=13, halign=:center, color=:gray30)
        
        weight_names = ["Height", "Shadow", "Volume", "MaxXY", "UpArea", "Down", "Gravity", "ExtSup", "IntSup"]
        
        row_idx = 1
        for (name, weight_obs) in zip(weight_names, weight_observables)
            row_grid = GridLayout(col3[row_idx, 1])
            Label(row_grid[1, 1], "$(rpad(name, 9)): ", fontsize=14, halign=:left, width=85, font=:bold)
            Label(row_grid[1, 2], "$(round(weight_obs[], digits=3))", fontsize=14, halign=:left, color=:gray30)
            row_idx += 1
            Label(col3[row_idx, 1], "", height=8)
            row_idx += 1
        end
        
        report_filename = "orientation_report_$(timestamp).png"
        save(report_filename, report_fig)
        println("Report saved to: $report_filename")
        
        screenshot_filename = "gui_screenshot_$(timestamp).png"
        save(screenshot_filename, main_figure)
        println("GUI screenshot saved to: $screenshot_filename")
    end
    
    export_stl_button = Button(main_controls_layout[row, 1], label="Export Oriented STL", width=240)
    row += 1
    on(export_stl_button.clicks) do _
        try
            seq = pattern_sequence_obs[]
            pos = clamp(orientation_control.value[], 1, length(seq))
            idx = seq[pos]
            displayed_mesh = oriented_meshes[idx]
            ts = replace(string(now()), ":" => "-", " " => "_")
            fname = "Export_Oriented_$(ts).stl"
            export_oriented_stl!(part_faces, displayed_mesh, fname)
            println("Exported STL: $fname")
        catch e
            @error "STL export failed" exception=(e, catch_backtrace())
        end
    end
    
    reset_view_button = Button(main_controls_layout[row, 1], label="Reset Camera View", width=240)
    row += 1
    on(reset_view_button.clicks) do _
        reset_limits!(part_view)
        part_view.azimuth[] = part_view.azimuth[]
        reset_limits!(cross_section_view)
        cross_section_view.elevation = π/2
        cross_section_view.azimuth = 0.0
        reset_limits!(performance_sphere_view)
        performance_sphere_view.azimuth[] = performance_sphere_view.azimuth[]
        reset_limits!(opposite_hemisphere_view)
        opposite_hemisphere_view.azimuth[] = opposite_hemisphere_view.azimuth[]
    end
    ...
    row += 1
    slice_height_control = Slider(main_controls_layout[row, 1], range=0.0:0.1:50.0, startvalue=25.0, width=240)
    row += 1
    ...
    Regenerates the external and internal support structures for a given orientation index and Z-offset value.
    If the offset is zero and the support type is "Lattice", it returns the original supports.
    """
    function regenerate_supports_with_offset(orientation_index, offset_value, current_support_type)
        ...
        ext_supports, int_supports, _, _ = identify_support_requirements(part_faces, offset_mesh, OVERHANG_THRESHOLD_DEGREES, SUPPORT_DIAMETER_MM, SUPPORT_DENSITY_REDUCTION, current_support_type)
    ...
    function update_spider_chart(orientation_index, weights)
        ...
        for (i, txt) in enumerate(weight_text_elements)
            txt.text = "w=$(round(weights[i], digits=2))"
        end
    end
    ...
    function update_3d_visualisation(target_orientation, display_mode)
        ...
        build_direction = orientation_vertices[target_orientation]
        up_vector = Point{3, Float64}(0.0, 0.0, 1.0)
        rotation = rotation_between(up_vector, normalizevector(build_direction))
        
        rotated_x = rotation * Point{3,Float64}(1.0, 0.0, 0.0)
        rotated_y = rotation * Point{3,Float64}(0.0, 1.0, 0.0)
        rotated_z = rotation * Point{3,Float64}(0.0, 0.0, 1.0)
        
        sync_x_line[1] = [origin, rotated_x]
        sync_y_line[1] = [origin, rotated_y]
        sync_z_line[1] = [origin, rotated_z]
        
        sync_x_text.position = rotated_x * 1.1
        sync_y_text.position = rotated_y * 1.1
        sync_z_text.position = rotated_z * 1.1
    ...
    on(orientation_control.value) do slider_position
        if mode_dropdown.selection[] == "Live Orientation"
            seq = pattern_sequence_obs[]
            actual_orientation = seq[min(slider_position, length(seq))]
            update_3d_visualisation(actual_orientation, "Live Orientation")
            current_orientation_line[1] = [slider_position]
        end
    end
    ...
    on(pattern_dropdown.selection) do selected_pattern
        current_pattern[] = selected_pattern
        new_sequence = create_pattern_sequence(orientation_vertices, selected_pattern)
        pattern_sequence_obs[] = new_sequence
        first_orientation = new_sequence[1]
        set_close_to!(orientation_control, first_orientation)
    end

    on(auto_play_button.clicks) do _
        is_auto_playing[] = !is_auto_playing[]
        auto_play_button.label = is_auto_playing[] ? "Stop Auto Play" : "Auto Play"
    end
    ...
    Rebalances the weights of the optimisation criteria based on locked states and updates the GUI accordingly.
    """
    function rebalance_all_weights()
    ...
    @async while true
        sleep(1.0)
        if is_auto_playing[]
            current_slider_pos = orientation_control.value[]
            max_pos = length(pattern_sequence_obs[])
            next_pos = current_slider_pos >= max_pos ? 1 : current_slider_pos + 1
            set_close_to!(orientation_control, next_pos)
        end
    end
    ...
    first_pattern_orientation = pattern_sequence_obs[][1]
    set_close_to!(orientation_control, 1)
    update_3d_visualisation(first_pattern_orientation, "Live Orientation")
    update_live_metrics(first_pattern_orientation)
    
    z_coords_init = [v[3] for v in oriented_meshes[first_pattern_orientation]]
    z_min_init, z_max_init = extrema(z_coords_init)
    slice_z_position[] = (z_min_init + z_max_init) / 2
    set_close_to!(slice_height_control, slice_z_position[])
    
    update_slice_visualisation(first_pattern_orientation, slice_z_position[], oriented_meshes, part_faces, 
                            slice_area, slice_plane_visualisation, slice_contour_data, 
                            show_slice_plane, cross_section_view)
...
function run_printing_orientation_analysis(test_case_number=3, window_title="PBF-LB Print Orientation Analysis", resolution=1.0, slice_resolution=50)
    ...
    interface = create_interactive_interface(analysis_results..., test_case_number)
